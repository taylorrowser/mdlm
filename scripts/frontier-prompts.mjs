function commonEditingPolicy(issue) {
  return `Read the complete issue and comments, parent spec, repository instructions, glossary, relevant ADRs, branch commits, and existing work. Treat issue #${issue.number} acceptance criteria as the current implementation boundary: parent invariants remain binding, but explicitly deferred sibling work remains deferred. Run focused checks and type checking while editing; do not run `npm test`, journey suites, or another Pi/code-review agent: the orchestrator independently owns the authoritative fast-suite run and independent review at the final committed tip. This instruction overrides the implementation skill's default completion procedure for validation and review. Commit completed work with issue #${issue.number} in the commit message. Work autonomously without asking for confirmation. Do not push, create or merge a PR, or close issues; the orchestrator owns publication.`;
}
