...
const REPRESENTATIVE_PREDICTED_MS = 223_861;
const REPRESENTATIVE_OBSERVED_SCHEDULER_WALL_MS = 265_550;
const REPRESENTATIVE_OBSERVED_WRAPPER_WALL_MS = 265_726;
const REPRESENTATIVE_TEST_WORK_MS = 574_610;
const REPRESENTATIVE_CONTENTION_ALLOWANCE_MS = Math.max(
  0,
  REPRESENTATIVE_OBSERVED_SCHEDULER_WALL_MS - REPRESENTATIVE_PREDICTED_MS,
);
...
const criticalContributors = rootTestManifest
  .filter((entry) => entry.runtimeClass === "process-repository-heavy"
    || entry.runtimeClass.startsWith("repository-public-"))
  .sort((left, right) => right.measuredDurationMs - left.measuredDurationMs || left.file.localeCompare(right.file))
  .slice(0, 10);
// Add the exact-current grouped representative gap once. Its first phase ran
// three safe-sensitive tasks with filler work; its second phase ran the heavy
// pair because heavy-sensitive overlap remains prohibited.
const modeledRootMs = simulation.wallMs
  + REPRESENTATIVE_CONTENTION_ALLOWANCE_MS
...
console.log(`root_files=${rootTestManifest.length} tasks=${tasks.length} token_capacity=${ROOT_TEST_TOKEN_CAPACITY} class_concurrency_limits=${JSON.stringify(ROOT_TEST_CLASS_CONCURRENCY_LIMITS)}`);
...
console.log(`representative_predicted_ms=${REPRESENTATIVE_PREDICTED_MS} representative_observed_scheduler_wall_ms=${REPRESENTATIVE_OBSERVED_SCHEDULER_WALL_MS} representative_observed_wrapper_wall_ms=${REPRESENTATIVE_OBSERVED_WRAPPER_WALL_MS} representative_test_work_ms=${REPRESENTATIVE_TEST_WORK_MS}`);
console.log(`representative_work_per_wall=${(REPRESENTATIVE_TEST_WORK_MS / REPRESENTATIVE_OBSERVED_SCHEDULER_WALL_MS).toFixed(3)} representative_contention_allowance_ms=${REPRESENTATIVE_CONTENTION_ALLOWANCE_MS}`);
