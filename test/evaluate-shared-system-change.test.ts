...
describe("shared accepted SYS package behavior", () => {
  ...
  it("exposes both exact consumers and preserves unrelated evidence after same-lineage replacement", () => {
    ...
    const consumerA = record("DWP", ids.a, dwp("API coverage"), [{type: "decomposes", target: system.datum.revision_id}], "complete-decomposition-work-package@3");
    const consumerB = record("DWP", ids.b, dwp("CLI coverage"), [{type: "decomposes", target: system.datum.revision_id}], "complete-decomposition-work-package@3");
