...
for (const {processName, corrections, partialAcceptance = false} of [
  ...
  test(`direct public ${processName}${corrections ? " correction" : ""}${partialAcceptance ? " partial acceptance" : ""} lifecycle preserves decisions, corrections and history`, async () => {
    ...
    const invocation = spawnSync(process.execPath,[path.join(process.cwd(),"scripts/direct-lifecycle-walkthrough.mjs"),"--process",processName,"--executable",executable,"--root",journeyRoot,...(corrections ? ["--corrections"] : []),...(partialAcceptance ? ["--partial-acceptance"] : [])],{
