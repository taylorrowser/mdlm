...
describe("evaluateLifecycle review flow", () => {
  ...
  it("projects the failed Phase 2 assurance chain through its existing correction modules", async () => {
    const stakeholderRequirement = record("STK", "STK-2A3B4C5D6E", {
    ...
    const requirement = record(
      ...
      { title: "System requirement" },
      {
        links: [{
          type: "derived-from",
          target: stakeholderRequirement.datum.revision_id,
        }],
      },
    );
    const strategy = record(
    ...
    const environment = record(
      ...
      { title: "System environment" },
      {
        links: [{
          type: "realizes",
          target: strategy.datum.revision_id,
        }],
      },
    );
    const activity = record(
      ...
      "VER-2A3B4C5D6E",
      {
        title: "System pilot activity",
        kind: "pilot",
        expected_success_activity: "Accept the supported case",
        expected_discrimination_activity: "Reject the unsupported case",
        claim: {
          kind: "pilot",
          scope: "verification-design",
          formal_evidence_eligible: false,
    ...
    const target = record(
      ...
      "ART-2A3B4C5D6E",
      {
        title: "Disposable system pilot target",
        kind: "prototype",
        supported_behavior: ["Accept the supported case"],
        unsupported_behavior: ["Reject the unsupported case"],
        prototype_controls: {
          activity_ref: activity.datum.revision_id,
        },
      },
      {
        links: [{ type: "derived-from", target: requirement.datum.revision_id }],
      },
    );
    const implementation = record(
      ...
      "VAI-2A3B4C5D6E",
      {
        title: "Source-blind system pilot procedure",
        kind: "pilot",
    ...
    const failedReview = (subject: LifecycleRecord, suffix: string) => {
      const context = record(
        "BSL",
        `BSL-2A3B4C5D${suffix}`,
        {
          title: "Exact assurance context",
          kind: "review-context",
          role: "review-context",
          scope: subject.datum.revision_id,
          group: "DEFAULT",
    ...
    const records = [
      ...
      target,
      implementation,
      ...failedReview(strategy, "1"),
      ...failedReview(environment, "2"),
      ...failedReview(activity, "3"),
      ...failedReview(implementation, "4"),
    ];
    const snapshot = {
      processRef: "git:phase-2-assurance-correction",
    ...
    expect(evaluation.looseEnds).toEqual(expect.arrayContaining(
      expectedCorrections.map(([obligation, subject, actionableResolver]) =>
      expect.objectContaining({
        obligation,
        subject,
        actionableResolver,
    ...
    for (const [obligation, subject, resolver] of expectedCorrections) {
      const correction = evaluation.looseEnds.find((item) =>
        item.obligation === obligation && item.subject === subject
      )!;
      ...
      expect(prepared.value.invocations).toHaveLength(1);
      expect(prepared.value.expectedOutputs.length).toBeGreaterThan(0);
    }
  });
