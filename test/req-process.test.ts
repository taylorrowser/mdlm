...
describe("req process package commands", () => {
  ...
  it("installs without implicit activation and records an exact selected package", async () => {
    ...
    expect(installation).toEqual({
      ...
      package: {
        id: "mdlm-bootstrap",
        version: "0.54.0",
        reference: "mdlm-bootstrap@0.54.0",
    ...
    expect(
      ...
      package: {
        ...
        path: ".lifecycle/packages/mdlm-bootstrap@0.54.0",
      },
  ...
  it("does not publish migration files when the installed target no longer validates", async () => {
    ...
    const installedObligation = path.join(
      repositoryRoot,
      ".lifecycle/packages/mdlm-bootstrap@0.54.0/obligations/review-context-required.yaml",
    );
    await fs.writeFile(
      installedObligation,
  ...
  it("validates the selected package and reports every validation surface", () => {
    ...
    expect(human.status, human.stderr).toBe(0);
    expect(human.stdout).toContain(
      "Validated Process Package: mdlm-bootstrap@0.54.0",
    );
  ...
  it("inspects the exact selected manifest and definition catalogs", () => {
    ...
    expect(JSON.parse(result.stdout)).toEqual({
      ...
      inspection: {
        ...
        definitionCatalogs: expect.objectContaining({
          ...
          phases: ["phase-0-wayfinding@4", "phase-1-product-assurance@5", "phase-2-pilot-assessment@2", "phase-2-system-definition@5", "phase-7-change-control@2"],
        }),
      },
      diagnostics: [],
    });
    ...
    for (const semantic of [
      "Process Package: mdlm-bootstrap@0.54.0",
