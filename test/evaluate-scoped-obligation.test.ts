...
const PROCESS_REF =
  "mdlm-bootstrap@0.56.0#sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
  ...
  it("authorizes and satisfies the initial Resolver through public req commands", async () => {
    ...
    try {
      ...
      expect(initial.id).toMatch(
        /^initial-map-required@1:phase-0-wayfinding@4:mdlm-bootstrap@0\.56\.0#sha256:[a-f0-9]{64}$/,
      );
