...
describe("req scenario execute", () => {
  ...
  beforeEach(async () => {
    ...
    obligation = `open-question-resolution@2:${question.revisionId}:mdlm-bootstrap@0.28.0#${packageDigest}`;
  ...
  it("invokes the adapter with validated bindings and atomically records exact provenance", async () => {
    ...
    expect(decision.lifecycleDatum.datum.created_by).toEqual({
      ...
      process_ref: expect.stringContaining("mdlm-bootstrap@0.28.0#sha256:"),
