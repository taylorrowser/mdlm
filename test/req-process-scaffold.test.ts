...
describe("req Process Package scaffolding", () => {
  ...
  it("scaffolds every accepted authored definition kind and updates its catalog", async () => {
    ...
    expect(await yaml(path.join(packageRoot, "scenarios/case-scenario.yaml")))
      .toEqual(expect.objectContaining({ initiation: "explicit", resolves: [] }));
