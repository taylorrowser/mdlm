...
inputs:
  ...
  - {name: subject_context, types: [BSL], cardinality: one, identity: revision}
outputs:
  - name: review
    types: [REV]
    cardinality: one
...
participation:
  ...
  arguments: {subject: plan, review_context: subject_context}
...
describe("Phase 2 earliest simplification through the public operator seam", () => {
  ...
  const seedFailure = async (
    ...
    adapterSequence += 1;
    const adapterPath = path.join(parent, `simplification-review-${adapterSequence}.mjs`);
    const response = {
      outputs: [{
        name: "review",
        invocation: 0,
        lifecycleDatum: {
          type: "REV",
          payload: {
            title: "Failed exact Phase 2 definition simplification",
            review_kind: "simplification-architecture-interfaces",
            decomposition_plan_revision: plan.revisionId,
            rubric_ref: "policies/rubrics/bootstrap-review.md@1",
            findings: [{
              id: "F-001",
              target: members[0]!.revisionId,
              relationship: "primary",
              severity: "blocking",
              summary: "The architecture has an unnecessary boundary.",
              evidence: "Both elements only relay the same report.",
            }, {
              id: "F-002",
              target: members[0]!.revisionId,
              relationship: "primary",
              severity: "blocking",
              summary: "The extra boundary duplicates responsibility.",
            }, ...members.slice(1).map((member, index) => ({
              id: `F-${String(index + 3).padStart(3, "0")}`,
              target: member.revisionId,
            ...
            definition_simplification: {
              ...
              scope_reduction: {
                removed_targets: [members.at(-1)!.revisionId],
                rationale: "One duplicated SYS behavior disappears with the unnecessary boundary.",
              },
            },
            outcome: "fail",
          },
          ...
          body: "The earliest exact definition set requires coherent scope reduction.\n",
        },
      }],
      completionEvidence: { summary: "The exact definition-set simplification failed." },
    };
    await fs.writeFile(
      adapterPath,
    ...
    const published = req(
      ...
      "--input",
      `subject_context=${context.revisionId}`,
      "--authorize",
  ...
  it("corrects all findings coherently, removes obsolete output work, and resumes completion only after fresh evidence", async () => {
    ...
    const context = freezeContext(exactSet, systemOne.revisionId);
    const failedReview = await seedFailure(plan, context, exactSet);
      ...
      data: {
        payload: {
          findings: expect.arrayContaining([
            expect.objectContaining({ id: "F-001", target: architecture.revisionId }),
            expect.objectContaining({ id: "F-002", target: architecture.revisionId }),
          ]),
        },
      },
    });
    ...
    const correctedSystemRevision = `${systemOne.id}-r00002`;
    const cause = { type: "corrects-review", target: failedReview.revisionId };
    const correction = submit(packet, [
