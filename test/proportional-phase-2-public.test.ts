...
import { runPhaseTwoAssuranceReviewRoute } from "./helpers/proportional-phase-2-routes.js";
import { runPhaseTwoSimplificationReviewBinding } from "./helpers/phase-2-simplification-review-binding.js";
import { runPhaseTwoReviewCorrectionRendering } from "./phase-2-review-correction-rendering.test.js";
...
it("groups one coherent stakeholder context into one ASP and DWP readiness route", async () => {
  ...
  const snapshot = (records: LifecycleRecord[]) =>
  ...
  const ready = (records: LifecycleRecord[], obligation: string) =>
    snapshot(records).obligations.filter(
      (item) => item.obligation === obligation && item.status === "ready",
    );
...
it("routes one reviewed Phase 2 completion directly to its level candidate", async () => {
  ...
  expect(
    processPackage.scenarios["create-decomposition-group-candidate"],
  ).toBeUndefined();
  expect(
    processPackage.obligations["decomposition-group-candidate-required"],
  ).toBeUndefined();
  ...
  expect(obligation.resolve_with).toEqual(
    expect.objectContaining({
