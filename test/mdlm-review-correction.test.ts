  ...
  it.each([
    ...
    for (const [cycleIndex, reviewOutcome] of replacementReviews.entries()) {
      ...
      if (reviewOutcome === "pass") {
        ...
        expect(packet.scenario.reference).toBe("define-verification-strategy@1");
        const strategyRequirements = exactInput(packet, "requirements").values;
        const strategyExecution = publish(packet, [{
          localId: "phase-1-strategy",
          name: "strategy",
          invocation: 0,
          lifecycleDatum: {
            type: "VSP",
            payload: {
              title: "Public command assurance strategy",
              rationale: "The accepted command intent needs exact positive and rejection evidence.",
              level: "stakeholder",
              ...
              evidence_policy: "Retain exact public inputs, outputs, and rejection observations.",
              assessment_policy: "Require deterministic success and discrimination evidence.",
              environment_profile: {
                id: "public-command",
                purpose: "Exercise the exact public command boundary.",
                capabilities: {
                  controllability: ["supply exact command vectors"],
                  observability: ["capture exit status, stdout, and stderr"],
                  external_services: [],
            ...
            links: [
              ...strategyRequirements.map((value: any) => ({
                type: "governs",
                target: value.identity.id,
              })),
              ...strategyRequirements.map((value: any) => ({
            ...
            body: "One exact Phase 1 public-boundary assurance strategy.\n",
          },
        }]);
        let strategy = strategyExecution.outputs[0].lifecycleDatum as {
          id: string;
          revisionId: string;
        };
        ...
        expect(packet.scenario.reference).toBe("create-review-context@1");
        expect(exactInput(packet, "subject").values[0].identity.revision_id)
          .toBe(strategy.revisionId);
        publishContext(packet);
        packet = nextPacket();
        expect(exactInput(packet, "subject").values[0].identity.revision_id)
          .toBe(strategy.revisionId);
        let failedStrategyReview = publishReview(packet, "fail")
          .outputs[0].lifecycleDatum as { revisionId: string };
        const strategyReviews = [failedStrategyReview.revisionId];

        for (let cycle = 1; cycle <= 2; cycle += 1) {
          const correctionOutcome = nextOutcome();
          expect(correctionOutcome).toEqual(expect.objectContaining({
            outcome: "assignment",
          }));
          packet = prepare(correctionOutcome);
          expect(packet.scenario.reference)
            .toBe("revise-verification-strategy-after-review@2");
          expect(packet.authority.requirements).toEqual([]);
          expect(exactInput(packet, "failed_reviews").values.map(
            (value: any) => value.identity.revision_id,
          )).toEqual([failedStrategyReview.revisionId]);
          expect(exactInput(packet, "failed_reviews").values[0].data.payload.findings)
            .toHaveLength(2);

          const replacementExecution = publish(packet, [{
            localId: `strategy-correction-${cycle}`,
            name: "replacement",
            invocation: 0,
            lifecycleDatum: {
              id: strategy.id,
              type: "VSP",
              payload: {
                title: `Corrected public command assurance strategy ${cycle}`,
                rationale: "The correction makes supported and malformed observations exact.",
                level: "stakeholder",
                permitted_methods: ["demonstration"],
                independence: {
                  boundary: "black-box",
                ...
                evidence_policy: "Retain byte-exact public success and rejection evidence.",
                assessment_policy: "Require distinct deterministic positive and malformed observations.",
                environment_profile: {
                  id: "public-command",
                  purpose: "Exercise the exact public command boundary.",
                  capabilities: {
                    controllability: ["supply exact supported and malformed command vectors"],
                    observability: ["capture exact exit status, stdout, and stderr bytes"],
                    external_services: [],
                    timing: "bounded deterministic timeout",
                  },
                },
              ...
              links: [
                ...
                {
                  type: "corrects-review",
                  target: failedStrategyReview.revisionId,
                },
              ],
              body: "The same strategy lineage carries every applicable failed Review.\n",
            },
          }]);
          strategy = replacementExecution.outputs[0].lifecycleDatum;

          packet = nextPacket();
          expect(packet.scenario.reference).toBe("create-review-context@1");
          expect(exactInput(packet, "subject").values[0].identity.revision_id)
            .toBe(strategy.revisionId);
          const freshContext = publishContext(packet).outputs[0].lifecycleDatum;
          packet = nextPacket();
          expect(exactInput(packet, "review_context").values[0].identity.revision_id)
            .toBe(freshContext.revisionId);
          failedStrategyReview = publishReview(packet, "fail")
            .outputs[0].lifecycleDatum;
          strategyReviews.push(failedStrategyReview.revisionId);
        }

        const strategyEscalation = nextOutcome();
        expect(strategyEscalation).toEqual(expect.objectContaining({
          outcome: "attention-required",
          authorityRequirement: expect.objectContaining({
            mode: "attended",
            authority: "stakeholder",
          ...
          attentionSchedule: expect.objectContaining({ timing: "immediate" }),
          explanation: expect.stringMatching(/both autonomous/i),
        }));
        packet = prepare(strategyEscalation);
        expect(packet.scenario.reference)
          .toBe("revise-verification-strategy-after-review@2");
        expect(exactInput(packet, "failed_reviews").values.map(
          (value: any) => value.identity.revision_id,
        ...
        for (const review of strategyReviews) {
          expect(invokeMdlm(repository, ["show", review, "--json"]).status).toBe(0);
        }
