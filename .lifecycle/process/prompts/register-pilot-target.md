...
source-blind implementer: an exact repository locator and an ordered public
command vector. For a parameterized CLI, use literal, checkout-path, and parameter
tokens. Keep each parameter name and its exact public encoding together in its
command token so no declared parameter can be disconnected from execution.

For a parameterized CLI, record one ordered command template that deterministically
instantiates the exact normal, raw-malformed, omitted-argument, and extra-argument
vectors. Co-locate each parameter's name and encoding with its four case tokens: a
supplied `value`, a raw malformed UTF-8 value where applicable, or an `omitted:
true` marker. The marker omits that declared parameter; a raw empty UTF-8 value
remains one supplied token.

For a zero-argument CLI, omit parameter tokens. Record exactly the normal
zero-argument case and one `extra-argument` case that represents the declared
rejection of every argument-bearing invocation. In either form, place each
extra-only raw token in command order with `extra_argument`. Literal and
checkout-path tokens participate in every vector, so repeated tokens cannot be
dropped or deduplicated by a case. For each case record the exact exit status and
base64 stdout/stderr bytes, and classify rejection cases as automatic rejection.
