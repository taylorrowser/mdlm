...
- **Evidence/observations:** `test/mdlm-review-correction.test.ts` drives VSP correction and exhausted-budget attention through `mdlm next`, `scenario prepare`, and `scenario submit`; `test/req-product-assurance-pilot.test.ts` preserves the exact ENV rebuild and positive/unsupported pilot evidence route; focused evaluator fixtures cover VSP, ENV, and pilot VER correction participation plus explicit ambiguity. The package advances to `mdlm-bootstrap@0.53.0`, `bootstrap@27`, and `phase-1-product-assurance@4`.
