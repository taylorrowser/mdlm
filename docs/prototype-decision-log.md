...
- **Status:** accepted and implemented
- **Decision:** Publish `mdlm-bootstrap@0.70.0` as a clean package break. Initial and corrected pilot VER authoring receive the exact current parent PSP Revision alongside the exact STK and VSP, and fresh VER Review Contexts preserve the same parent support. The PSP may resolve relative requirement terms such as supported argument count, accepted units, or closed value sets, but cannot introduce product behavior absent from the accepted intent. Keep every valid completed RUN as durable procedure history. A pilot RUN satisfies its Obligation and Phase 1 progression only when it produces a valid pilot RES with `outcome: suitable` and both expected success and expected discrimination observed.
