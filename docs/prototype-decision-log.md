...
- **Evidence/observations:** `test/mdlm-stakeholder-change-control.test.ts` uses the compiled public `mdlm next`, `scenario prepare`, and `scenario submit` process for every lifecycle transaction after focused setup. It rejects unaccepted and under-reported impact plus wrong-lineage replacement, exercises every attended disposition, routes failed CHG/disposition/STK/candidate Reviews through Correction, distinguishes affected Stale evidence from exact reusable evidence, preserves accepted history, and closes only after fresh replacement and candidate Reviews. `test/mdlm-review-correction.test.ts` retains the pre-acceptance public Correction route. The package advances to `mdlm-bootstrap@0.57.0`, `bootstrap@32`, and `phase-7-change-control@3`.
