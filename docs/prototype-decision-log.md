...
- **Evidence/observations:** `test/mdlm-vai-correction.test.ts` drives malformed target registration, deterministic exact-byte observations, omitted-versus-empty-token identity, duplicate-token order, multi-Review correction, changed-binding rejection with no partial publication, immutable prior RUN/RES evidence, fresh context and Review, and exact fresh-run selection through `mdlm next`, `scenario prepare`, and `scenario submit`. `test/req-product-assurance-pilot.test.ts` retains the complete qualification and pilot chain. The package advances to `mdlm-bootstrap@0.54.0`, `bootstrap@28`, and `phase-1-product-assurance@5`.
