...
- **Status:** accepted and implemented
- **Decision:** Keep the acceptance boundary and complete change behavior in the Example Process Package. Ordinary Phase 0 Correction excludes exact Revisions already contained by an accepted baseline. Phase 7 CHG impact accepts one STK only when that exact Revision is a member of an authorized `intent-approved` accepted baseline, then requires exact typed impact, contextual Review, one attended approve/reject/defer/cancel Decision, and independent Decision Review. Rejection, deferral, and cancellation close without replacement. Approval alone permits one same-lineage `changed-under` STK replacement, ordinary fresh Review Context and Review for that exact Revision, one CHG-linked replacement candidate preserving all unaffected accepted definitions/evidence, fresh candidate Review, and exact atomic closure.
...
- **Expected behavior:** Draft STK Review failure receives ordinary autonomous Correction with no CHG. A CHG naming an unaccepted STK or wrong exact Revision fails atomically. Reviewed impact projects immediate stakeholder attention; the Decision is inapplicable until its Review passes. Reviewed reject/defer/cancel ends the exact request with no STK r00002 and preserves accepted maturity. Reviewed approve derives only a same-lineage replacement, fresh exact context/Review, selective candidate and candidate Review, then closure; missing evidence remains a normal blocker through `mdlm next`.
...
- **Evidence/observations:** `test/mdlm-stakeholder-change-control.test.ts` uses the compiled public `mdlm next`, `scenario prepare`, and `scenario submit` process for every lifecycle transaction after focused setup. It rejects unaccepted impact and wrong-lineage replacement, exercises every attended disposition and required Decision Review, preserves accepted history, projects exact reusable definitions/evidence, and closes only after fresh replacement and candidate Reviews. `test/mdlm-review-correction.test.ts` retains the pre-acceptance public Correction route. The package advances to `mdlm-bootstrap@0.57.0`, `bootstrap@32`, and `phase-7-change-control@3`.
