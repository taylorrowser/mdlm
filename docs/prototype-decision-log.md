...
- **Decision:** Publish `mdlm-bootstrap@0.73.0` as a fresh-only package break. An answered initial product-intent QST stores one normalized, self-contained `attended_answer`, not a transcript. The initial DEC must retain it exactly. Each same-lineage attended correction receives that exact QST authority and must either retain the answer or declare an explicit narrow, defer, or remove change with rationale. Corrected Decision Review Contexts include the exact Question lineage, source boundary, answer authority, prior Decision, and causal failed Review. `compile-psp@3` receives the accepted current DEC as its complete product-intent authority.
