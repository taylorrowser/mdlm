...
- **Status:** accepted and implemented
- **Decision:** Every current failed or valid collateral-flagged SYS, ASP, ICSP, and planning/completion DWP Review derives one subject-bounded same-lineage Correction carrying all exact causes. One Phase 2 participation Policy permits two autonomous Review-correction cycles before immediate attended escalation. Unsupported multiple-current single-valued architecture, strategy, or planning cardinality derives attended exact cancellation and independent Review rather than arbitrary selection; every valid member of a nonempty multi-interface set remains available to planning. DWP planning and completion use source-owned links as their sole exact parent, output, architecture, interface, strategy, plan, and simplification account; every parent is mechanically covered by SYS derivation, and duplicate payload identities are invalid. A reviewed gate rejection may implicate the level candidate or one exact SYS/ASP/ICSP/DWP member. Local member Correction and fresh Review precede serial affected-group and level replacements, each preserving unaffected members/evidence and recording the predecessor through `supersedes`. A reviewed approving level gate then derives one mechanical flattened `level-accepted` baseline before its existing exact gate evidence authorizes pilot-assessment progression.
...
- **Expected behavior:** Every supported unfinished Phase 2 state projects Assignment or Attention Required. Plural valid interfaces do not create false ambiguity. Invalid or contradictory completion accounts publish nothing through initial completion or later Correction. Routine local correction receives fresh Review; a third failure requires attended scope authority. A reviewed member rejection cannot approve or progress: it derives exact member Correction, fresh Review, affected group replacement and Review, then level replacement and Review before returning Attention Required to the same gate. Approval remains insufficient until its DEC Review passes and the exact accepted baseline publishes; progression then reuses that reviewed gate evidence without redundant attention.
