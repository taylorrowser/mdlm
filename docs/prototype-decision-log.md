...
## D-073 — Bind a Review Context to one exact primary subject

- **Status:** accepted and implemented
- **Decision:** A `review-context` BSL uses `payload.scope` as the exact primary subject Revision. `create-review-context@1` completion requires that identity, and `valid-review-contexts-for@1` selects a context only when its scope equals the supplied subject. Supporting parents, siblings, decisions, and evidence remain exact members without becoming additional primary subjects.
- **Alternatives:** Keep treating every definition member as a primary subject; omit parents from child contexts; force Review of every parent before child authoring; pick an arbitrary context when `one(...)` is ambiguous; or encode primary identity in the kernel's baseline capability.
- **Rationale:** The clean #70 pilot created a PSP context before deriving STKs, then correctly included the PSP parent in each STK context. Membership-only context selection made the PSP Review binding ambiguous and produced a null queue. Primary Review scope is package meaning already represented by BSL payload, while exact membership and freezing remain kernel capability behavior.
- **Expected behavior:** Multiple child contexts may contain the same parent without competing with the parent's own context. Each subject resolves exactly its scoped Review Context; unrelated or support-only membership cannot satisfy Review Context or passing-Review work. The prompt and Scenario contract reject a mismatched scope before publication.
- **Reversibility:** Another package may model primary Review identity differently through its own baseline payload and Selector. Generic kernel interfaces and historical context bytes remain unchanged.
- **Evidence/observations:** `test/req-phase-0-wayfinding.test.ts` freezes the PSP context before STK derivation, then creates an STK context containing the exact PSP parent, and proves PSP Review remains unambiguous and Dispatchable. `test/req-scenario-execution.test.ts` rejects a mismatched primary scope atomically. The clean pilot migrated publicly from `0.42.0` to `0.43.0` at commit `42918da`, immediately rediscovered the formerly ambiguous PSP Review, and autonomously published passing `REV-R2JRXHKQ5F-r00001` at `9d2e427`. The package advances to `mdlm-bootstrap@0.43.0` and `bootstrap@17`.

## D-074 — Failed Phase 1 assurance Reviews derive autonomous exact correction

- **Status:** accepted and implemented
- **Decision:** A current failed Review of a Phase 1 VSP, ENV, or pilot VER derives package-owned same-lineage correction work. Each correction takes every exact current failed REV as input, links `corrects-review` to every and only those Reviews, preserves the failed Revision and Review immutably, and requires a fresh exact Review Context and independent Review. ENV correction atomically publishes the replacement ENV plus a new qualification VER/VAI pair; prior qualification RUN/RES evidence cannot qualify the replacement Revision.
- **Alternatives:** Leave `req next` null after a valid failed Review; require stakeholder intervention; use explicit initiation of the original realization Scenario; mutate the failed artifact; reuse the prior environment result; narrow capability claims silently; or add generic lifecycle vocabulary to MDLM core.
- **Rationale:** Independent Review findings are expected package feedback, not stakeholder choices. The clean pilot correctly found that two identical stateless controls could not substantiate independent execution, but the package had no correction Resolver and stranded five pilot implementation obligations. Exact package-owned correction closes that queue seam while retaining reviewer independence, evidence-class separation, and immutable history.
- **Expected behavior:** Failed current VSP, ENV, and pilot VER subjects project a Dispatchable correction Scenario. Missing exact failed-Review linkage rejects the whole output batch. A corrected ENV is not eligible for pilot work until its newly authored qualification implementation runs, the exact replacement assurance chain is frozen, and a fresh independent Review passes. No stakeholder authority is requested.
- **Reversibility:** Additional packages or later lifecycle slices may define narrower correction contracts. The generic kernel remains unchanged; removing these Example Process Package rules would restore the previous null queue without rewriting historical Lifecycle Data.
- **Evidence/observations:** `test/req-product-assurance-pilot.test.ts` reproduces failed ENV Review, asserts the actionable public correction projection, rejects an incomplete correction atomically, publishes ENV r00002 with a fresh qualification chain, preserves the failed REV and both ENV Revisions, executes new qualification evidence, and requires fresh context and Review before pilot implementation. The clean #70 pilot exposed the original null queue at `7fbd098` through failed `REV-NR8BZ9CC5B-r00001`, migrated publicly to `0.44.0` at `d3c9e56`, published corrected ENV r00002 and fresh qualification design at `e498fe7`, recorded state-sensitive qualification evidence at `100d5c9`, and passed fresh independent Review `REV-9FRVQFV89D-r00001` at `996917d`. The package advances to `mdlm-bootstrap@0.44.0` and `bootstrap@18`.

## D-075 — Register existing exact pilot targets through discoverable work

- **Status:** accepted and implemented
- **Decision:** Every current exact Phase 1 requirement with current pilot verification activity work derives `pilot-target-required@1`. When no current target exists, `register-pilot-target@1` records one existing immutable Git commit as ART with bounded supported and intentionally unsupported behavior and an exact `derived-from` requirement Revision link. Exactly one target satisfies the Obligation; multiple current targets remain blocked as explicit ambiguity.
- **Alternatives:** Require a prototype-bound QST even when the prototype already exists; use generic `req new ART`; infer the root Git commit privately; let implementation select arbitrarily among zero or multiple targets; reuse an ENV as the product target; or treat target registration as stakeholder authorization.
- **Rationale:** ART registration records existing repository evidence and its bounded execution surface; it does not choose new product scope or accept a requirement. The clean pilot imported the exact prototype before process initialization, so QST-only ART publication left all five pilot implementations with unresolved `execution_target` and another null queue.
- **Expected behavior:** Missing target evidence projects an actionable Resolver through `req next`. The operator confirms the full Git object resolves as a commit and retains that observation; mutable syntax, absent observation, empty behavior bounds, and mismatched exact requirement linkage reject atomically. After one valid ART publishes, repeated execution of the satisfied Obligation is rejected and pilot implementation can bind the singular exact target. No stakeholder turn is introduced.
- **Reversibility:** A later repository-evidence adapter may automate immutable commit inspection while preserving the exact ART, requirement link, bounds, and ambiguity rules. Generic core and historical data remain unchanged.
- **Evidence/observations:** `test/req-product-assurance-pilot.test.ts` resolves an existing repository HEAD, then covers public missing-target work, `req next`, mutable-ref, absent-observation, empty-bound, and mismatched-link atomic rejection, valid ART execution, duplicate execution rejection, and resulting pilot implementation Dispatchability. The clean #70 pilot exposed the missing target seam after corrected ENV Review at `996917d`, migrated publicly to `0.45.0` at `772923b`, registered five exact requirement-scoped prototype targets in commits `d0b7ee4` through `6e306d7`, passed doctor after every transaction, and immediately projected source-independent implementation work. The package advances to `mdlm-bootstrap@0.46.0` and `bootstrap@20`.

## D-076 — Pilot targets carry their complete controlled public execution interface

- **Status:** accepted and implemented
- **Decision:** An ART eligible as a Phase 1 pilot target records, in addition to its immutable commit and behavior bounds, the exact repository locator, versioned typed public command vector whose parameter tokens co-locate names and exact encodings, fresh-temporary-directory isolation contract, and observable success/rejection protocols. `current-pilot-targets-for-requirement@1` treats only boundary-complete ART evidence as eligible, so migration makes older incomplete registrations discoverable work without rewriting them.
- **Alternatives:** Let the independent implementer read product source, unit tests, `PILOT.md`, or operator memory; infer an invocation from an entrypoint filename; publish an abstract non-executable VAI; put product-specific CLI facts in generic MDLM code; or ask the stakeholder how to run an already bounded prototype.
- **Rationale:** A target commit and behavioral scope identify what to assess but not how a source-blind agent can reach and observe its public interface. The complete interface belongs with package-owned ART evidence. Keeping it there makes the downstream implementation module deeper: its dry-run supplies every execution fact while generic Scenario execution remains neutral.
- **Expected behavior:** Registration rejects incomplete public-interface output atomically. A fresh independent implementer can derive exact positive and discrimination cases from only public dry-run/show/schema projections, without prohibited inputs or stakeholder attention. Historical ART bytes remain unchanged; they simply do not satisfy current boundary-complete target selection.
- **Reversibility:** Other Process Packages may define different target-interface shapes. A future repository adapter may materialize checkouts or runners while preserving these exact declarative inputs.
- **Evidence/observations:** In the clean #70 pilot at `652b16a`, a fresh no-tools implementer discarded the first proposal because public projections omitted the executable, argument vector, observation protocol, and repository locator. After an intermediate `0.47.0` migration and five boundary registrations, a second fresh implementer could write a complete procedure but incorrectly invented full-word scale tokens where the public interface requires `C` and `F`; that proposal was also discarded before publication. `test/req-product-assurance-pilot.test.ts` protects incomplete-boundary rejection and verifies that the versioned exact public interface appears in the downstream implementation dry-run. The clean pilot migrated to the final contract at `2468fa2`, registered five encoded targets in `30d5497` through `d35a82b`, and a third fresh no-tools implementer then produced the exact correct `F`/`C` command vectors and autonomously published `VAI-4W6X8P2K7M-r00001` plus authorization `DEC-5Y7Z9Q3R8N-r00001` at `e52d3a1`. Final review then exposed that separate parameter and encoding collections could drift and that command vectors must allow duplicate tokens, so interface v2 co-locates each parameter with its encoding and treats the command as an ordered non-unique sequence. The clean pilot migrated to the final typed-command contract at `0ab057c`, retained every historical ART byte through successful doctor, and registered five v2 targets at `ee1b403` through `70527fd`. The package advances to `mdlm-bootstrap@0.49.0` and `bootstrap@23`.

## D-096 — Bound Phase 1 assurance correction and stop unsupported ambiguity explicitly

- **Status:** accepted and implemented
- **Decision:** Current failed VSP, ENV, and pilot VER Reviews use one type-specific correction interface that carries every exact failed Review and Finding applicable to the current Revision; the existing shared correction-history Selector counts completed correction cycles. Two replacement-and-fresh-Review cycles are autonomous; continued failure changes the same Scenario participation to immediate attended stakeholder authority and requires one exact scope DEC. Strategy Review precedes ENV and pilot VER fan-out, and assurance tied to an earlier exact strategy cannot satisfy a corrected VSP. Corrected ENV work always authors a fresh qualification VER/VAI chain, while corrected VSP and pilot VER Revisions receive fresh exact contexts and independent Reviews. Multiple applicable strategies, environments, or pilot targets are an explicit Profile Boundary in the narrow bootstrap profile; missing or incomplete evidence remains ordinary Obligation, Question, or validation work.
- **Alternatives:** Retry assurance correction without limit; introduce one generic kernel correction type; add separate escalation Scenarios for each assurance type; reuse prior ENV qualification evidence; allow downstream work before strategy Review; silently choose among multiple strategies, environments, or targets; or leave unsupported ambiguity as an accidental dead end.
- **Rationale:** Routine assurance findings do not warrant stakeholder interruption, but an unbounded autonomous loop is not live. Carrying every current blocking Review keeps one Assignment complete, while the existing shared correction-history Selector, one narrow authority-evidence Selector, and one Phase 1 participation Policy keep the budget and authority seam consistent without moving V-model vocabulary into the kernel. The correction Obligations only report that correction is ready; they do not duplicate budget thresholds or attention rules, leaving the participation Policy as the canonical authority decision. Reviewing VSP before dependent authoring prevents avoidable fan-out, while exact strategy bindings prevent stale assurance reuse. The bootstrap does not yet contain enough disposition breadth to resolve imported ambiguity safely, so an explicit successful boundary is more truthful than arbitrary selection.
- **Expected behavior:** The public operator returns autonomous same-lineage correction for initial and first-replacement failures, then Attention Required with complete lineage and findings after the second replacement fails. A correction missing any applicable current `corrects-review` cause rejects atomically. ENV replacement cannot borrow old qualification RUN/RES evidence. Unavailable empirical evidence still routes through exact QST source freezing and evidence-provider attention; bounded intentionally unsupported target behavior remains required discrimination evidence.
- **Reversibility:** A broader Process Package may replace the Phase 1 ambiguity boundary with reviewed disposition Questions or Decisions while preserving all exact history. The package-owned budget and links can change without adding lifecycle IDs or correction vocabulary to MDLM core.
- **Evidence/observations:** `test/mdlm-phase-1-assurance-correction.test.ts` drives VSP, ENV, and pilot VER correction, exact causal-link rejection, evidence reuse, exhausted-budget attention, self-contained attended packets, and all three unsupported ambiguity classes through `mdlm next`, `scenario prepare`, and `scenario submit`; it also proves an attended Phase 1 replacement preserves both autonomous cycles. `test/req-product-assurance-pilot.test.ts` retains detailed exact ENV rebuild and positive/unsupported pilot evidence checks. The package advances to `mdlm-bootstrap@0.53.0`, `bootstrap@27`, and `phase-1-product-assurance@4`.

## D-097 — Correct source-blind pilot procedures against exact command evidence

- **Status:** accepted and implemented
- **Decision:** A current failed Review of a source-blind pilot VAI derives one same-lineage correction carrying every exact failed Review and Finding. The replacement preserves exact VER, ENV, ART, declared-case, target-behavior, and pilot claim-class bindings while allowing procedure and activity-binding text to address the Review. Its procedure records bounded checkout, environment-check, and product-case deadlines as infrastructure safety only; process-group termination, forced kill and descendant reaping; partial raw observation; guaranteed cleanup; and continue-through-all-cases aggregation. ART public interfaces carry an ordered typed command matrix that instantiates exact normal, raw-malformed, omitted-argument, and extra-argument cases with case-specific exact exit status and base64 stdout/stderr bytes. Parameter encodings are shared across cases by construction, omitted markers belong to declared parameters, and raw empty tokens remain supplied.
- **Alternatives:** Retry the failed VAI without a new Revision; mutate or reuse prior RUN/RES evidence; let implementers infer malformed variants from prose; model omission as an empty token; deduplicate command tokens; treat timeout as a product timing requirement; stop after the first timeout; or add VAI correction behavior to MDLM core.
- **Rationale:** The pilot's source-blind Reviews correctly identified unsafe procedures and ambiguous malformed-input evidence. These are package feedback, not stakeholder choices. Keeping exact command cases on ART and exact correction behavior in package Selectors, Obligations, Policies, Scenarios, prompts, and type schemas deepens the package-owned module while the kernel remains lifecycle-neutral.
- **Expected behavior:** The first two failed replacement cycles are autonomous and further failure uses the existing attended Phase 1 escalation policy. Missing causes or changed VER, ENV, ART, declared-case, target-behavior, or claim-class bindings reject the complete proposal atomically; reviewed procedure and activity-binding corrections remain allowed. A valid correction receives fresh exact Review Context and independent Review; prior VAI, authorization, RUN, RES, context, and Review evidence stays immutable and cannot satisfy the replacement's run Obligation. Timeout recovery retains partial observation, guarantees cleanup, and continues later cases without making a product timing claim.
- **Reversibility:** Another Process Package may define different procedure or command-case schemas and correction budgets without changing generic MDLM interfaces. Exact historical Lifecycle Data remains interpretable under its recorded package.
- **Evidence/observations:** `test/req-product-assurance-pilot.test.ts` drives malformed target registration, deterministic exact-byte observations, omitted-versus-empty-token identity, and duplicate-token order. `test/mdlm-phase-1-assurance-correction.test.ts` drives multi-Review correction, changed-binding rejection with no partial publication, immutable prior RUN/RES evidence, fresh context and Review, and exact fresh-run selection through `mdlm next`, `scenario prepare`, and `scenario submit`. The package advances to `mdlm-bootstrap@0.54.0`, `bootstrap@28`, and `phase-1-product-assurance@5`.

## D-098 — Correct the earliest exact Phase 2 definition set before fan-out

- **Status:** accepted and implemented
- **Decision:** Deepen the generic Review Context module so the first exact planning DWP, SYS output, ASP, and ICSP set—and no unrelated member—is its complete simplification boundary before DWP completion, candidate assembly, or gate authority. A failed simplification Review nests all primary Findings under one exact target, binds collateral Findings collectively to the complete canonical set, and carries either one exact current SYS blocker or every and only member of a declared atomic definition-consistency set. One SYS subject with all of its findings receives one serial same-lineage correction; architecture, interface, or planning-DWP findings require the coherent set. That correction publishes same-lineage ASP, every ICSP, planning DWP, and only still-applicable SYS replacements, each citing the exact failed REV through source-owned `corrects-review`. DWP and SYS interface bindings have one canonical representation in source-owned exact links rather than duplicate payload arrays. Exact `removes` links may remove a proper subset: removed lineages have no successor, every retained lineage has exactly one replacement, and no new lineage is admitted. A judgment that the accepted parent requires no SYS output returns typed inability because parent-scope cancellation is outside this correction. Every replacement receives fresh contextual Review and both complete-set simplification judgments before completion resumes.
- **Alternatives:** Keep the existing pass-only Reviews; simplify after DWP completion or candidate assembly; dispatch one correction per Finding; mutate exact references piecemeal; retain duplicate ICSP payload/link representations with comparison Selectors; preserve removed outputs in the replacement DWP; model complete parent-scope cancellation and terminal DWP disposition here; add a generic kernel correction or lifecycle status; or pull Phase 2 gate and broader failure behavior forward from issue #99.
- **Rationale:** The first concrete definition set is the earliest evidence-bearing place where independent judgment can distinguish necessary system behavior from architecture-created obligations. Correcting there avoids multiplying challenged scope into completion, candidates, and gate evidence. A subject-bounded route keeps independent findings serial, while the explicit complete-set route is justified only because changing ASP, ICSP, or planning DWP identity requires their exact dependent references to move atomically. All routing remains package-authored and MDLM core remains lifecycle-neutral.
- **Expected behavior:** A contract-valid failure cannot satisfy simplification. `mdlm next` projects one exact correction Assignment containing complete Finding evidence. Removed SYS output remains immutable history but no longer belongs to the replacement plan's current output set, so no downstream completion or candidate work is fabricated for it. Fresh exact contexts, Reviews, and simplification evidence unblock the existing DWP completion Obligation through normal reevaluation.
- **Reversibility:** A broader Process Package may declare narrower interface consistency sets, additional architecture groups, correction budgets, or attended escalation without changing the kernel. Historical exact Revisions and failed judgments remain preserved under their recorded package.
- **Evidence/observations:** `test/mdlm-phase-2-simplification.test.ts` drives actual simplification Assignments, rejects cross-subject and all-output-removal contracts, corrects one SYS with several Findings, then routes a coherent ASP/ICSP/DWP/SYS failure, rejects duplicate stale ICSP payload references, proves the ordinary planning-DWP Review Assignment receives the complete corrected definition set, requires exact causal links and fresh evidence, and resumes completion through `mdlm next`, `scenario prepare`, and `scenario submit`. `test/req-system-decomposition.test.ts` retains the complete Phase 2 candidate journey. The package advances to `mdlm-bootstrap@0.55.0`, `bootstrap@29`, and `phase-2-system-definition@6`.

## D-099 — Close the supported Phase 2 failure and gate routes

- **Status:** accepted and implemented
- **Decision:** Every current failed or exact collateral-flagged SYS, ASP, ICSP, and planning/completion DWP Review derives one subject-bounded same-lineage Correction carrying all exact causes. One Phase 2 participation Policy permits two autonomous Review-correction cycles before immediate attended escalation. Unsupported multiple-current single-valued architecture, strategy, or planning cardinality derives attended exact cancellation and independent Review rather than arbitrary selection; every valid member of a nonempty multi-interface set remains available to planning. DWP planning and completion use source-owned links as their sole exact parent, output, architecture, interface, strategy, plan, and simplification account; every parent is mechanically covered by SYS derivation, and duplicate payload identities are invalid. Failed or reviewed-rejected group and level candidates receive same-lineage frozen replacements that preserve exact members, evidence, and composition while `supersedes` records the predecessor. A reviewed Phase 2 gate rejection canonically blocks the exact level candidate. A reviewed approving level gate then derives one mechanical flattened `level-accepted` baseline before its existing exact gate evidence authorizes pilot-assessment progression.
- **Alternatives:** Add one kernel correction engine; author separate correction Scenarios for every Phase 2 type; infer collateral routing only from prose; choose the latest ambiguous datum; accept incomplete payload-only coverage summaries; mutate rejected candidates; restart Phase 2 after rejection; treat Review or gate approval as requirement acceptance; or ask for a second progression Decision.
- **Rationale:** One deep package-owned correction interface keeps failure behavior complete without moving lifecycle nouns or budgets into MDLM core. Canonical exact links remove mismatch states from completion. Evidence-preserving candidate replacement and mechanical acceptance separate immutable judgment, candidacy, acceptance, and progression while reusing the one reviewed authority decision.
- **Expected behavior:** Every supported unfinished Phase 2 state projects Assignment or Attention Required. Plural valid interfaces do not create false ambiguity. Invalid or contradictory completion accounts publish nothing through initial completion or later Correction. Routine local correction receives fresh Review; a third failure requires attended scope authority. A reviewed candidate rejection cannot approve or progress and returns an evidence-preserving superseding candidate to the same gate. Approval remains insufficient until its DEC Review passes and the exact accepted baseline publishes; progression then reuses that reviewed gate evidence without redundant attention.
...
- **Status:** accepted and implemented
- **Decision:** Keep the pilot-assessment interface package-owned and deepen it through one bounded PAS Correction route, one attended Expansion Decision Correction route, and package-authored outcome declarations. A failed current PAS receives two autonomous same-lineage replacements over the unchanged exact `pilot-assessment-context`, each citing its failed REV and receiving fresh independent Review; continued failure or stakeholder-owned findings change the same Scenario participation to attended authority and require one exact scope DEC. Only a current passing PAS exposes the nondelegable stakeholder Expansion Decision. That DEC must adopt the PAS recommendation, cite the PAS and passing PAS Review exactly, and pass independent Review; failure derives an attended same-lineage DEC replacement preserving all exact evidence and causal Review links. Reviewed `proceed` reaches Profile Boundary because Phases 3–6 are omitted, reviewed `change` progresses into the declared Phase 7 change-control route and reaches Profile Boundary only after supported work drains, and reviewed `stop` declares Lifecycle Complete.
- **Alternatives:** Treat any recorded Decision as terminal; leave PAS Correction unbounded and autonomous; let an earlier passing PAS or Decision dominate a newer failure; infer stop from rejection; map every recommendation to one Profile Boundary; add Phase 3–6 placeholders; route outcomes in generic TypeScript; or introduce a second operator interface.
- **Rationale:** Exact currentness, causal same-lineage evidence, independent judgment, and explicit terminal declarations prevent a failed assessment or Decision from authorizing expansion and prevent intentional stop from appearing as Process Dead End. Reusing the established public Assignment seam and generic terminal classifier keeps PAS, Review, Decision, correction budget, and Phase IDs out of MDLM core. Progressing `change` to the existing change-control Phase gives later accepted-requirement tickets one declared route without implementing their explicitly deferred breadth here.
...
- **Status:** accepted and implemented
- **Decision:** Keep the acceptance boundary and complete change behavior in the Example Process Package. Ordinary Phase 0 Correction excludes exact Revisions already contained by an accepted baseline. Phase 7 CHG impact accepts one STK only when that exact Revision is a member of an authorized `intent-approved` accepted baseline, then records that STK and baseline as exact roots from which the complete directly traced set is derived, requires contextual Review, one attended approve/reject/defer/cancel Decision, and independent Decision Review. Rejection, deferral, and cancellation close without replacement. Approval alone permits one same-lineage `changed-under` STK replacement, ordinary fresh Review Context and Review for that exact Revision, one CHG-linked replacement candidate preserving all unaffected accepted definitions/evidence, fresh candidate Review, and exact atomic closure. Failed CHG, replacement, and candidate Reviews share one deep package Correction interface; failed disposition Review renews attended judgment.
- **Alternatives:** Treat Review as acceptance; let ordinary correction revise accepted STK; infer impact from every backlink; approve through chat or before CHG/Decision Review; model each disposition as a separate workflow; reuse a prior Review for the replacement; rebuild the whole accepted baseline; mutate accepted history; retain the old unaccepted one-SYS tracer; or implement the shared two-consumer SYS sibling early.
- **Rationale:** Exact baseline membership distinguishes reviewed draft quality from durable authorization. One package-owned disposition interface and one evidence-complete candidate seam keep lifecycle nouns out of the kernel while preventing unauthorized accepted-intent replacement. Keeping `changed-under` on the replacement and candidate makes the causal change queryable, while exact Review targets make fresh context and Review evidence derivable without altering the generic Review module; selecting reusable accepted items from the bounded impact avoids whole-phase restart. Shared accepted SYS propagation remains explicitly deferred to #102.
- **Expected behavior:** Draft STK Review failure receives ordinary autonomous Correction with no CHG. A CHG naming an unaccepted STK, a non-containing baseline, extra impact roots, or the wrong exact Revision fails atomically; directly affected evidence derives from valid roots and cannot be omitted. Reviewed impact projects immediate stakeholder attention; the Decision is inapplicable until its Review passes. Reviewed reject/defer/cancel ends the exact request with no replacement and preserves accepted maturity. Reviewed approve derives only a same-lineage replacement, explicit Staleness for impacted evidence, fresh exact context/Review, selective candidate and candidate Review, then closure. Any failed required Review routes to causal same-lineage Correction and fresh Review through normal `mdlm next`; missing closure evidence remains a normal blocker.
- **Reversibility:** Another package may choose different accepted baseline kinds, impact breadth, dispositions, candidate shape, or closure evidence over the same generic baseline, graph, Scenario, Assignment, participation, and Operator Outcome primitives. Immutable prior Revisions and Decisions preserve migration options.
- **Evidence/observations:** `test/mdlm-stakeholder-change-control.test.ts` uses the compiled public `mdlm next`, `scenario prepare`, and `scenario submit` process for every lifecycle transaction after focused setup. It rejects unaccepted and wrong-baseline impact roots plus wrong-lineage replacement, exercises every attended disposition, routes failed CHG/disposition/STK/candidate Reviews through Correction, distinguishes affected Stale evidence from exact reusable evidence, preserves accepted history, and closes only after fresh replacement and candidate Reviews. `test/mdlm-review-correction.test.ts` retains the pre-acceptance public Correction route. The package advances to `mdlm-bootstrap@0.57.0`, `bootstrap@32`, and `phase-7-change-control@3`.

## D-102 — Protect one shared accepted system requirement across exact consumers

- **Status:** accepted and implemented
- **Decision:** Keep one SYS Stable Datum lineage across consumers. Each consumer is one exact DWP Revision whose `decomposes` link binds the exact SYS Revision and whose own completion payload records complete coverage. A draft SYS replacement derives serial lightweight consumer reevaluation without CHG. Exact membership in a frozen `level-accepted` baseline moves substantive SYS replacement behind reviewed impact, attended approve/reject/defer/cancel disposition, same-lineage `changed-under` replacement, one serial same-lineage DWP update and fresh Review per affected consumer, a selective changed-under level candidate, and exact closure.
- **Alternatives:** Copy SYS requirements per consumer; infer impact from mutable latest identities; update both consumers atomically or in parallel; rebuild all system evidence; add multi-group scheduling; or generalize change behavior to future requirement levels.
- **Rationale:** Exact source-owned links already provide the deep package seam: shared identity, consumer binding, impact traversal, and selective evidence reuse need no kernel concept. Serial consumer Obligations keep coordination visible and bounded while each DWP retains independent coverage judgment.
- **Expected behavior:** Two consumers of one exact SYS are always both visible. Pre-acceptance replacement reopens each consumer without CHG. Post-acceptance impact derives all consumers and dependent context, Review, candidate/gate, and verification evidence; only passing-reviewed attended approval permits replacement. Each consumer then rebinds and receives fresh Review before candidate and closure. Unrelated accepted evidence stays Valid and reusable.
- **Reversibility:** Another Process Package may model consumers or acceptance differently over the same generic exact links, baselines, Selectors, Obligations, Scenarios, and serial Assignment contract.
- **Evidence/observations:** `test/mdlm-shared-system-change-control.test.ts` proves complete two-consumer impact through the public compiled operator seam. `test/evaluate-shared-system-change.test.ts` proves both consumer Obligations and selective Staleness/reuse. The package advances to `mdlm-bootstrap@0.59.0`, `bootstrap@34`, `phase-2-system-definition@8`, and `phase-7-change-control@4`.

## D-103 — Make planning-DWP independent Review evidence-complete

- **Status:** accepted and implemented
- **Decision:** A current planning-DWP Review freezes and projects every and only exact allocated ASP, governing ICSP, and applicable current SYS support beside the DWP. The package accepts the prompt-mandated `simplification-product-definition` pass, fail, or cancellation for that DWP; failure targets and blocks only the exact plan. Authenticated thin DWP Review Contexts from exact 0.63.0 and 0.64.0 remain usable without rewriting frozen data only when they contain solely the DWP and no evidence or composed baselines.
- **Alternatives:** Let reviewers infer linked definitions from raw package or repository access; continue sending an empty packet; accept a contextual Review contrary to the prompt; rewrite frozen contexts during migration; or broadly trust every historical thin or augmented context.
...
## D-107 — Route candidate correction behind exact Question disposition

- **Status:** accepted and implemented
- **Decision:** Publish `mdlm-bootstrap@0.68.0` as a clean package break. A failed intent candidate cannot dispatch correction while one of its exact definition members references a current unresolved Question. That Question receives immediate package-declared attention when it blocks the failed candidate. After the Question has an applicable authorized disposition, candidate correction receives the exact current QST Revision and every applicable reviewed DEC alongside the existing candidate, member, Review, lineage, and gate-rejection inputs.
- **Alternatives:** Mutate the published 0.67.0 package identity; globally prioritize all attended work in the kernel; infer Question disposition from Review prose; require an outgoing `blocks` link that cannot target a not-yet-created candidate; or let candidate correction answer the Question autonomously.
- **Rationale:** Candidate membership and validated outbound references provide a package-evaluable causal relation without teaching the kernel Phase, QST, BSL, or Scenario names. Blocking correction preserves stakeholder authority, while supplying exact reviewed disposition evidence after resolution makes reassembly possible without inference or repeated insufficient-input Assignments.
- **Expected behavior:** An indexed open preferential Question makes its dependent candidate correction non-dispatchable and projects attended `resolve-question@2` work immediately. Once an exact scoped Decision has passed its required independent Review, correction becomes dispatchable with the resulting Question and Decision in its prepared packet. Candidates with no such dependency retain their existing correction route.
- **Compatibility:** None with 0.67.0 repositories, Assignments, or historical Lifecycle Data. Existing demonstrations remain immutable 0.67.0 evidence and must be archived; any continuation under 0.68.0 starts from a fresh repository rather than migrating or rewriting those runs.
- **Evidence/observations:** Public `evaluateLifecycle` and next-work projection coverage reproduces candidate → member → indexed open Question → failed candidate Review, proves attended Question resolution is selected first, then supplies an exact reviewed scope Decision and proves the candidate can be reassembled and independently reviewed without another correction loop. Existing failed-candidate regression coverage proves routes without Question dependencies still dispatch correction.

## D-108 — Review correction deltas before Phase 0 fan-out

- **Status:** accepted and implemented
- **Decision:** Publish `mdlm-bootstrap@0.69.0` as a clean package break. Require a passing independent Review of the exact current PSP before STK derivation becomes dispatchable. When a Phase 0 foundation correction requires stakeholder authority, the existing scope DEC records a structured comparison among the smallest bounded correction, deferral or removal, and explicit retention of broader behavior; defer-or-remove records an exact reactivation condition. The corrected MAP, PSP, or STK receives one fresh ordinary Review Context containing the exact prior lineage, causal failed Reviews and gate rejections, and applicable correction Decision. Ephemeral author preflight and correction prompts compare the proposal delta and stop rather than infer stakeholder-visible scope. Late whole-candidate simplification remains unchanged as defense in depth.
- **Alternatives:** Rely on stronger prompt prose alone; use keyword, line, artifact, or lifecycle budgets as complexity heuristics; prioritize Review ahead of STK work only in the operator; add an unconditional second simplification Review; publish author-preflight evidence; add a new scope-expansion Datum or lifecycle level; or combine Review Context/Assignment lease optimization with this semantic change.
- **Rationale:** The preserved calculator lineage showed locally reasonable determinism corrections accumulating into globally disproportionate binary64 conversion and rendering machinery. Generic simplicity prose and attended authority did not expose the correction delta to fresh independent judgment soon enough. Exact causal context and structural fan-out blocking catch that risk without reducing lifecycle semantics, fabricating authority, or teaching the kernel Phase 0 concepts. A local temperature-converter output/failure inconsistency additionally shows why bounded correction must remain available without triggering machine-representation detail.
- **Expected behavior:** An unreviewed current PSP cannot dispatch STK drafting. Attended correction atomically rejects an unframed scope Decision and accepts an exact bounded, defer-or-remove, or retain disposition with complete comparative options and necessity. Corrected foundation Review sees only causally relevant prior Revisions, Reviews, gate rejections, and correction authority. Advancing a stable PSP parent makes an existing STK Review Context stale until it is rematerialized against the current PSP. Ordinary bounded clarification remains local; explicitly necessary complexity can be retained through exact stakeholder authority; and late candidate simplification can still reject a disproportionate whole set.
- **Compatibility:** None with 0.68.0 repositories, Assignments, or historical Lifecycle Data. The published 0.68.0 package and its clean-sheet demonstrations remain immutable. New 0.69.0 work starts from a fresh repository; existing 0.68.0 runs must be completed under 0.68.0 or archived and restarted rather than migrated or rewritten.
- **Evidence/observations:** Repository-backed public regressions execute `mdlm next`, `scenario prepare`, and `scenario submit` to prove PSP-before-STK routing, atomic rejection of legacy unframed correction, successful bounded correction, and fresh corrected-subject Review. Approved evaluator seams prove exact comparative context, exclusion of unrelated Decisions, current-child staleness, and bounded, defer-or-remove, and retain progression. A matched disposable route reduced the pre-Review checkpoint from 13 to 10 transactions, 14 to 11 Revisions, 6 to 4 Review Contexts, and 22,148 to 17,113 Lifecycle Data bytes; each route had 3 Reviews, 1 correction loop, 1 repository load, and 0 stale Assignment occurrences. The comparative packet increased from 50,834 to 81,003 bytes; single-run command latency increased from 812.35 ms to 935.34 ms and is not a budget or benchmark claim. Review Context reuse, batching, and stale-lease friction remain a separate optimization question.

## D-109 — Supply exact pilot intent and require exercised behavior

- **Status:** accepted and implemented
- **Decision:** Publish `mdlm-bootstrap@0.70.0` as a clean package break. Initial and corrected pilot VER authoring receive the exact current parent PSP Revision alongside the exact STK and VSP. Each pilot VER records that PSP Revision with an exact `derived-from` link, and fresh VER Review Contexts preserve the recorded parent support. The PSP may resolve relative requirement terms such as supported argument count, accepted units, or closed value sets, but cannot introduce product behavior absent from the accepted intent. Keep every valid completed RUN as durable procedure history. A pilot RUN satisfies its Obligation and Phase 1 progression only when it produces a valid pilot RES with `outcome: suitable` and both expected success and expected discrimination observed.
- **Alternatives:** Let activity authors infer relative terms; supply product source; duplicate all parent values into each STK; treat every completed procedure as successful evidence; add a new RUN state for setup failure; discard inconclusive RUN/RES history; or combine the change with operator resumability, performance optimization, or Review Context batching.
- **Rationale:** Source-independent verification design still needs the accepted definitions that give relative STK terms exact meaning. Procedure completion is separate from exercising product behavior. RUN and RES already represent that distinction, so package selectors can preserve setup failure, safe refusal, timeout, and all-not-launched evidence without allowing those outcomes to prove pilot suitability.
- **Expected behavior:** Prepared initial and corrected activity packets include the exact parent PSP and prohibit contract broadening. A pilot activity tied to a superseded PSP no longer satisfies the current requirement's activity Obligation. A completed setup-failure or all-not-launched RUN with an inconclusive RES remains valid and queryable, leaves the run Obligation unsatisfied, and offers a fresh bounded execution. A completed pilot advances only after suitable evidence records both expected success and expected discrimination.
- **Compatibility:** None with 0.69.0 repositories, Assignments, or historical Lifecycle Data. The published 0.69.0 package and its demonstrations remain immutable. New 0.70.0 trials start from fresh repositories; existing runs are completed under their recorded package or archived rather than migrated or rewritten.
- **Evidence/observations:** Repository-backed regressions use public `mdlm next`, `scenario prepare`, and `scenario submit` to prove exact PSP packet support, accepted source-independent activity publication, durable all-not-launched submission, and fresh retry allocation. Approved evaluator coverage proves setup-failure evidence blocks progression, suitable exercised evidence permits progression, correction receives the same parent support, and unrelated product behavior remains prohibited.

## D-110 — Preserve correction authority through candidate Review

- **Status:** accepted and implemented
- **Decision:** Complete the fresh-only `mdlm-bootstrap@0.70.0` break by carrying exact Phase 0 correction support into later Review Contexts. A current intent candidate Review receives each corrected member's prior lineage, causal failed Review or gate rejection, and trusted attended correction Decision. It also receives the latest exact QST and structurally valid Decision for every Question referenced by a current candidate member. A correction Decision Review receives the exact corrected member and causal failure, while a Question Decision Review receives its exact effective-scope QST. Gate readiness, progression, and completion remain blocked until every correction or Question Decision relied upon by the candidate has a valid passing Review.
- **Alternatives:** Ask later reviewers to infer authority from corrected prose; copy Decision rationale into each corrected member; include every historical Decision and Review; accept a candidate while relied-upon authority has a failed Review; or add Phase 0 behavior to the kernel or operator.
- **Rationale:** Fresh 0.69 trials showed a locally reviewed correction passing with attended authority and then failing candidate Review because the later packet omitted that authority. Another trial reached gate work while the Decision authorizing corrected scope had a failed Review. Exact package selectors can preserve only the current causal records and keep the kernel package-neutral.
- **Expected behavior:** Retain and defer-or-remove corrections reach candidate Review with their exact current authority and causes. A preferential Question resolution reaches candidate correction and the replacement candidate Review as an exact QST and Decision pair. Review instructions do not reopen an attended choice merely because earlier prose predates its Decision, but still reject contradictions, excess scope, absent authority, and unresolved Questions. A failed correction-authority Review can produce a same-lineage replacement, but that replacement must preserve the prior disposition and any defer-or-remove reactivation condition. Unrelated, stale, and superseded Decisions remain excluded. A clean first-pass route adds no correction Decision.
- **Compatibility:** No migration from 0.69.0. Published 0.69 repositories and demonstration history remain immutable. This decision joins D-109 in the same prospective 0.70.0 package and must pass combined validation before publication.
- **Evidence/observations:** Public repository regressions cover retained scope, defer-or-remove with an exact reactivation condition, and preferential Question resolution through `mdlm next`, `scenario prepare`, and `scenario submit`. Focused evaluator coverage proves exact candidate and Decision Review support, unrelated support exclusion, and gate blocking when relied-upon authority lacks a passing Review.

## D-111 — Classify correction authority in every failed Review

- **Status:** accepted and implemented
- **Decision:** Publish `mdlm-bootstrap@0.71.0` as a fresh-only package break. Every failed Review records `correction_authority: stakeholder` when any blocking finding needs a new choice or change to stakeholder-owned intent, or `correction_authority: package-evidence` when exact supplied package evidence determines the complete correction without that choice. Omission is invalid. Package Selectors and Policies grant autonomous correction only to explicit `package-evidence` failures. Existing `stakeholder` failures still route immediate attended foundation escalation with one required Decision.
- **Alternatives:** Keep omission as an autonomous default; route every failure to attended correction; infer authority from finding prose; let each Correction Scenario rediscover the boundary; or hard-code Review vocabulary in the runtime.
- **Rationale:** The 0.70 calculator canary published a Review whose finding could not be resolved without changing fixed stakeholder intent, but omission sent it to an autonomous Assignment with no truthful proposal. The temperature canary showed that existing routing works when the marker is present. Classification belongs in Review evidence because the independent reviewer has the exact subject, context, and blocking finding set needed to identify the authority boundary.
- **Expected behavior:** Publishing a failed Review without classification is rejected atomically. A stakeholder-owned failure immediately yields attended correction and exact Decision output. A package-evidence failure remains eligible for the bounded autonomous correction budget. The independent Review prompt, skill, rubric, Scenario completion condition, REV schema, Selectors, and Policies agree on the distinction.
- **Compatibility:** No migration from 0.70.0. Published 0.70 repositories, Assignments, and demonstration history remain immutable. New work starts in a fresh 0.71.0 repository.
- **Evidence/observations:** Public `mdlm next`, `scenario prepare`, and `scenario submit` regressions reject omitted classification before publication and accept explicit package-evidence correction. Public schema and evaluator coverage prove both routing branches, immediate attention, and required Decision output.

## D-112 — Resolve initial product intent before PSP compilation

- **Status:** accepted and implemented
- **Decision:** Publish `mdlm-bootstrap@0.72.0` as a fresh-only package break. Initial wayfinding must create and index one preferential product-intent QST. The package freezes that source through `freeze-source-boundary@1`, obtains an immediate attended stakeholder answer through `resolve-question@2`, and requires a passing Review of the resulting exact scope Decision before `compile-psp@3` becomes dispatchable. PSP compilation receives that Decision as an exact input and must link its output to it.
- **Alternatives:** Prefer PSP work through ready-item ordering alone; infer intent from a repository name, ambient `AGENTS.md`, or source files; add a runtime source bypass; make PSP compilation attended without durable authority evidence; or trust prompt prose while retaining an empty Assignment packet.
- **Rationale:** The 0.71.0 calculator canary dispatched autonomous PSP compilation with no exact inputs or authority evidence while the package had an unresolved preferential product-intent QST. One worker returned truthful inability, while another published unsupported product scope. Package ordering exposed the defect, but only an exact input and publication contract prevents it.
- **Expected behavior:** A fresh route reaches attended product-intent resolution before PSP compilation. The PSP Assignment contains the exact current reviewed scope Decision, and submission without the required `derived-from` link fails atomically. Low-cost reversible clarification remains autonomous only within the resolved product boundary.
- **Compatibility:** No migration from 0.71.0. Published 0.71 repositories, Assignments, and demonstration evidence remain immutable. New work starts in a fresh 0.72.0 repository.

## 2026-08-21 — Preserve normalized attended product answers through Decision correction

- **Decision:** Publish `mdlm-bootstrap@0.73.0` as a fresh-only package break. An answered initial product-intent QST stores one normalized, self-contained `attended_answer`, not a transcript. The initial DEC must retain it exactly. Each same-lineage attended correction receives that exact QST authority and must either retain the answer or declare an explicit narrow, defer, or remove change whose exact previous and revised answers match the QST and replacement Decision. Corrected Decision Review Contexts include the exact Question lineage, source boundary, answer authority, prior Decision, and causal failed Review. `compile-psp@3` receives the accepted current DEC as its complete product-intent authority.
- **Reason:** The 0.72 route could lose supplied formulas, invalid cases, exclusions, and simplicity constraints during Decision correction while still satisfying structural completion. PSP compilation then treated those attended details as absent.
- **Compatibility:** No migration from 0.72.0. Published 0.72 repositories, Assignments, and demonstration evidence remain immutable. New work starts in a fresh 0.73.0 repository.
- **Evidence/observations:** A bounded public regression covers `mdlm init`, `next`, `scenario prepare`, and `scenario submit` through source freezing, attended resolution, Decision Review, exact PSP preparation, and publication without repository-name or ambient-file inference.

## 2026-08-21 — Rebuild stale Phase 0 candidates after product answers

- **Decision:** Keep `mdlm-bootstrap@0.73.0` fresh-only and make checkpointed product Questions causal. A Question records stable `blocks` links to its governing PSP and only affected STKs. A passing reviewed answer makes any candidate containing uncorrected members in those lineages stale. Same-lineage replacements incorporate the exact answer Decision; unrelated STKs are not revised. Review waits until every affected current member incorporates the answer, then the original candidate lineage is superseded with current reviewed members before gate work can resume.
