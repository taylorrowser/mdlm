...
- **Status:** accepted and implemented
- **Decision:** Every current failed or exact collateral-flagged SYS, ASP, ICSP, and planning/completion DWP Review derives one subject-bounded same-lineage Correction carrying all exact causes. One Phase 2 participation Policy permits two autonomous Review-correction cycles before immediate attended escalation. Unsupported multiple-current single-valued architecture, strategy, or planning cardinality derives attended exact cancellation and independent Review rather than arbitrary selection; supported multi-interface sets remain intact. DWP completion uses source-owned links as its canonical exact parent, output, architecture, interface, plan, and simplification account. Failed or reviewed-rejected group and level candidates receive same-lineage frozen replacements that preserve unaffected exact members, evidence, and composition while `supersedes` records the predecessor. A reviewed approving level gate then derives one mechanical flattened `level-accepted` baseline before its existing exact gate evidence authorizes pilot-assessment progression.
...
- **Expected behavior:** Every supported unfinished Phase 2 state projects Assignment or Attention Required. Invalid completion accounts publish nothing. Routine local correction receives fresh Review; a third failure requires attended scope authority. A reviewed rejection cannot approve or progress and returns a superseding candidate to the same gate. Approval remains insufficient until its DEC Review passes and the exact accepted baseline publishes; progression then reuses that reviewed gate evidence without redundant attention.
...
- **Evidence/observations:** `test/evaluate-bootstrap-participation.test.ts` covers explicit attended ambiguity, supported interface multiplicity, and exhausted reviewed-rejection correction budget; `test/mdlm-phase-2-simplification.test.ts` retains public correction behavior; and `test/req-system-decomposition.test.ts` rejects incomplete exact composition and carries reviewed gate evidence through mechanical system acceptance and compiled `mdlm` progression outcomes. The package advances to `mdlm-bootstrap@0.56.0`, `bootstrap@30`, and `phase-2-system-definition@7`.
