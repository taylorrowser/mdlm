...
- **Evidence/observations:** `test/mdlm-review-correction.test.ts` drives VSP correction and exhausted-budget attention through `mdlm next`, `scenario prepare`, and `scenario submit`; `test/mdlm-phase-1-assurance-correction.test.ts` drives ENV and pilot VER correction, evidence reuse, exhausted-budget attention, and unsupported ambiguity through that same public operator seam and proves an attended Phase 1 replacement preserves both autonomous cycles. `test/req-product-assurance-pilot.test.ts` retains detailed exact ENV rebuild and positive/unsupported pilot evidence checks. The package advances to `mdlm-bootstrap@0.53.0`, `bootstrap@27`, and `phase-1-product-assurance@4`.
