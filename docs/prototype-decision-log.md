...
- **Status:** accepted and implemented
- **Decision:** Freeze the first exact planning DWP, SYS output, ASP, and ICSP set—and no unrelated member—as the complete simplification boundary before DWP completion, candidate assembly, or gate authority. A failed simplification Review nests all primary Findings under one exact target, binds collateral Findings to the complete canonical set, and carries either one exact current SYS blocker or every and only member of a declared atomic definition-consistency set. One SYS subject with all of its findings receives one serial same-lineage correction; architecture, interface, or planning-DWP findings require the coherent set. That correction publishes same-lineage ASP, ICSP, planning DWP, and only still-applicable SYS replacements, each citing every failed REV through source-owned `corrects-review`. Exact `removes` links require omitted scope while every undeclared output lineage remains represented. Every replacement receives fresh contextual Review and both complete-set simplification judgments before completion resumes.
