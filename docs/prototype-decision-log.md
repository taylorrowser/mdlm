...
- **Evidence/observations:** `test/mdlm-phase-2-simplification.test.ts` drives actual simplification Assignments, rejects a cross-subject contract, corrects one SYS with several Findings, then routes a coherent ASP/ICSP/DWP/SYS failure, rejects retained removed scope, requires exact causal links and fresh evidence, and resumes completion through `mdlm next`, `scenario prepare`, and `scenario submit`. `test/req-system-decomposition.test.ts` retains the complete Phase 2 candidate journey. The package advances to `mdlm-bootstrap@0.55.0`, `bootstrap@29`, and `phase-2-system-definition@6`.
