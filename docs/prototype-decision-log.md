...
- **Status:** accepted and implemented
- **Decision:** Freeze the first exact planning DWP, SYS output, ASP, and ICSP set as the complete simplification boundary before DWP completion, candidate assembly, or gate authority. A failed simplification Review records typed primary and collateral Findings plus canonical exact blockers. One SYS subject with all of its findings receives one serial same-lineage correction; architecture, interface, or planning-DWP findings may declare the complete exact definition set as one atomic consistency boundary. That correction publishes same-lineage ASP, ICSP, planning DWP, and only still-applicable SYS replacements, each citing every failed REV through source-owned `corrects-review`. Every replacement receives fresh contextual Review and both complete-set simplification judgments before completion resumes.
...
- **Evidence/observations:** `test/mdlm-phase-2-simplification.test.ts` drives multi-Finding failure, one coherent ASP/ICSP/DWP/SYS correction, exact causal links, actual output scope reduction, fresh acceptable Review of every retained output, fresh complete-set simplification, and resumed completion through `mdlm next`, `scenario prepare`, and `scenario submit`. `test/req-system-decomposition.test.ts` retains the complete Phase 2 candidate journey. The package advances to `mdlm-bootstrap@0.55.0`, `bootstrap@29`, and `phase-2-system-definition@6`.
