...
`npm test` is the normal authoritative gate. It builds once, verifies every Vitest file is classified exactly once, runs each root task through the weighted process scheduler, and runs controller tests. The four-token root schedule assigns all 27 process, repository, public, and compiled resource owners to three fixed, deterministic per-file LPT lanes. Each resource file runs in its own isolated Vitest process with `--maxWorkers=1`; resource work is never batched. The fourth token is available only to canonical fixture filler and cheap work. Descriptive runtime classes do not decide resource ownership: in particular, `load-scenario-participation` remains a resource owner even though its runtime class is canonical/evaluator. Heavy and fragile work use explicit mutually incompatible barrier waves, while safe resource work fills all three lanes. No more than three resource tasks or four total tasks may run at once.

The durable event table and barrier-aware lane plan are in `scripts/root-test-resource-plan.mjs`. It retains successful observations separately from failed, canceled, timed-out, and unadmitted evidence; only successful observations can supply selected estimates. Its default process budget is exactly 10 minutes. On the current four-logical-CPU, two-physical-core SMT host, preflight evidence must model the complete root gate at 590 seconds or less and leave at least 10 seconds below the exact 600-second process deadline. The model includes one measured mixed-work allowance and orchestration allowance rather than hiding resource work beneath another lane. A `NO_GO_MODEL_BLOCKER` result prohibits an authoritative launch. Do not describe a small observation set as p95 evidence.
