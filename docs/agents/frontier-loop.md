...
`npm run test:release` is release and demo authority. It retains all 46 root files, including every release-only compiled-public and complete-lifecycle route, then runs the same package and controller checks. `npm run test:all` aliases this release gate. Run it once on the final integrated main candidate before artifact publication or a demonstration, not after every narrow correction.

Both gates use the weighted process scheduler. The complete release schedule assigns all 26 process, repository, public, and compiled resource owners to deterministic lanes in three ordered phases: two heavy plus one safe, one fragile plus two safe, then three safe tails. A barrier separates each phase. Each resource file runs in its own isolated Vitest process with `--maxWorkers=1`; resource work is never batched. The fourth token is available only to evaluator, canonical fixture filler, and cheap work. Descriptive runtime classes do not decide resource ownership: `load-scenario-participation` remains a resource owner, while only `evaluate-scoped-obligation` moves to fourth-token evaluator work. Heavy is capped at two owners, fragile at one, safe at three, and heavy never overlaps fragile. No more than three resource tasks or four total tasks may run at once.

The durable event table and barrier-aware phase plan are in `scripts/root-test-resource-plan.mjs`. It retains successful observations separately from failed, canceled, timed-out, and unadmitted evidence. Only successful observations can supply selected task estimates. The final three-safe tail uses a compatibility-tested partition rather than greedy LPT. Two exact current-host attempts reached the former 1,150,000 ms deadline with production work incomplete, and a later exact final-main attempt reached 2,000,000 ms after all 47 root files and 105 package tests passed but before the controllers completed. The complete model therefore includes an explicit 1,200,000 ms censored-host variance allowance without treating those failures as successful timing. The production partition must model at 2,100,000 ms or less. The complete model covers both builds, suite verification, package tests, and both controller test commands; it must fit a 2,350,000 ms launch target and leave at least 50,000 ms below the exact 2,400,000 ms process deadline. A `NO_GO_MODEL_BLOCKER` result prohibits an authoritative launch. Do not describe a small observation set as p95 evidence.
...
`vitest.suites.mjs` assigns each root file a minimum authority gate. `pr` files run in both gates; `release` files run only in the complete release gate unless a PR names one as a focused regression. Suite verification rejects a missing, duplicate, stale, or invalid qualification classification. The release gate is a strict superset of the PR gate, so moving a route out of PR does not move it out of release acceptance.

The first successful PR-gate calibration on this host completed in 492,870 ms with status 0, below its 600,000 ms bound. `scripts/qualification-gate-evidence.mjs` records its exact tested content tree. It keeps successful observations separate from failed or censored observations.

Environment overrides:

- `MDLM_TEST_BUDGET_MS` — selected gate process budget override; PR defaults to `600_000` milliseconds and release defaults to `2_400_000` milliseconds;
