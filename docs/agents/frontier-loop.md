...
The durable event table and barrier-aware phase plan are in `scripts/root-test-resource-plan.mjs`. It retains successful observations separately from failed, canceled, timed-out, and unadmitted evidence; only successful observations can supply selected estimates. The calibrated heavy phase is 181,042 ms for its whole two-heavy-plus-one-safe window, including the Phase 1 contract delta; it is not combined with a separate contention allowance. Its safe lane is Phase 1, pilot assessment, and phase evaluation. The final three-safe tail uses an exact minimax partition rather than greedy LPT. The default process budget is exactly 10 minutes. On the current four-logical-CPU, two-physical-core SMT host, preflight evidence must model the complete root gate at 590 seconds or less and leave at least 10 seconds below the exact 600-second process deadline. The model reports both ideal and compatibility/barrier lower bounds and includes each phase calibration once plus the orchestration allowance. A `NO_GO_MODEL_BLOCKER` result prohibits an authoritative launch. Do not describe a small observation set as p95 evidence.
