...
`npm test` is the normal authoritative gate. It builds once, verifies every Vitest file is classified exactly once, runs each root task through the weighted process scheduler, and runs controller tests. The four-token root schedule reserves three deterministic LPT lanes for the 19 safe files and one serial background lane for heavy, fragile, canonical-filler, and cheap work. Every safe file gets its own focused Vitest process with `--maxWorkers=1`; no safe lane reuses a multi-file process. Safe tasks retain process/repository versus canonical/evaluator ownership metadata, but both ownership classes may use any safe lane because max-3 evidence produced no cleanup or semantic resource conflict. Heavy and fragile tasks may each overlap safe work, but remain mutually incompatible and never overlap each other in the background lane. Its default process budget is exactly 10 minutes. On the current four-logical-CPU, two-physical-core SMT host, preflight evidence must model the complete root gate at 590 seconds or less and leave at least 10 seconds below the exact 600-second process deadline. Canceled, failed, timeout, and unadmitted observations are status evidence only, never successful timing inputs. Do not describe a small observation set as p95 evidence.
