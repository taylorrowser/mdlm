...
The durable event table and barrier-aware phase plan are in `scripts/root-test-resource-plan.mjs`. It retains successful observations separately from failed, canceled, timed-out, and unadmitted evidence. Only successful observations can supply selected task estimates. The final three-safe tail uses a compatibility-tested partition rather than greedy LPT. Two exact current-host attempts reached the former 1,150,000 ms deadline with six or fewer production tasks unlaunched, so the complete model now includes an explicit 800,000 ms censored-host variance allowance without treating those failures as successful timing. The production partition must model at 1,700,000 ms or less. The complete model covers both builds, suite verification, package tests, and both controller test commands; it must fit a 1,950,000 ms launch target and leave at least 50,000 ms below the exact 2,000,000 ms process deadline. A `NO_GO_MODEL_BLOCKER` result prohibits an authoritative launch. Do not describe a small observation set as p95 evidence.
...
- `MDLM_TEST_BUDGET_MS` — authoritative gate process budget; defaults to exactly `2_000_000` milliseconds;
