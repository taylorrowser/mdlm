...
| Consolidated checkpoint conversation | `test/operator-outcome.test.ts` | One compiled-public-seam `attention-required` outcome exposes the complete compatible Consolidation Group; the root normalizes both conclusions, publishes each exact QST/DEC transaction serially through `scenario submit`, reevaluates between them, and stores no transcript. |
